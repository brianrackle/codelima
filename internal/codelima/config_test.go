...
func TestDefaultConfigYAMLIncludesGlobalLimaCommandsWithComment(t *testing.T) {
	...
	if !strings.Contains(output, "start: '{{binary}} start -y {{instance_name}}'") {
...
`
	...
	if got := cfg.LimaCommands.Start; got != "{{binary}} start -y {{instance_name}}" {
		t.Fatalf("expected default start command, got %q", got)
	}
	if got := cfg.LimaCommands.Create; got != "{{binary}} create -y --name {{instance_name}} --cpus=2 --memory=4 --disk=20 {{template_path}}" {
		t.Fatalf("expected default create command, got %q", got)
	}
	if got := cfg.LimaCommands.WorkspaceSeedPrepare; got != `sudo rm -rf {{target_path}} && sudo mkdir -p {{target_parent}} && sudo chown "$(id -un)":"$(id -gn)" {{target_parent}}` {
		t.Fatalf("expected default workspace seed prepare command, got %q", got)
	}
	if got := cfg.LimaCommands.Clone; got != "{{binary}} clone -y {{source_instance}} {{target_instance}}" {
