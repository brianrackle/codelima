...
// Run the actual publication shell with a recording GitHub CLI to verify
// regular release policy, qualification notes, and the complete asset set.
func TestReleaseWorkflowPublishesChannelAndAssets(t *testing.T) {
	...
	for _, tag := range []string{"v0.3.0", "v1.2.3"} {
		t.Run(tag, func(t *testing.T) {
			...
			cmd.Env = append(os.Environ(), "PATH="+filepath.Join(root, "bin")+":"+os.Getenv("PATH"), "GH_RECORD="+record, "GH_REPO=brianrackle/codelima", "TAG="+tag)
			...
			if !strings.Contains(string(args), "--notes-file\n.github/release-notes/"+tag+".md\n") {
				t.Errorf("missing qualification notes: %s", args)
			}
			...
			}
			{
				...
				if output, err := missingNotes.CombinedOutput(); err == nil {
					t.Fatalf("published release without qualification notes: %s", output)
				}
				if _, err := os.Stat(record); !os.IsNotExist(err) {
					t.Fatalf("GitHub CLI was called without release notes: %v", err)
				}
			}
		})
	}
...
func TestReleaseWorkflowPromotesStableAndRemovesBeta(t *testing.T) {
	workflow := loadReleaseWorkflow(t)
	var script string
	...
	root := testutil.TempDir(t, "release-tap-")
	...
	installerWrite(t, filepath.Join(seed, "Formula/codelima-beta.rb"), "beta formula\n", 0o644)
	...
	installerWrite(t, filepath.Join(root, "tmp/release/generated/Formula/codelima.rb"), "promoted formula\n", 0o644)
	for range 2 {
		...
		cmd.Env = append(os.Environ(), "GIT_CONFIG_NOSYSTEM=1", "GIT_CONFIG_GLOBAL=/dev/null",
			...
			"HOMEBREW_TAP_TOKEN=test-token", "TAG=v0.3.0",
		...
		if got := git("--git-dir", origin, "show", "main:Formula/codelima.rb"); got != "promoted formula\n" {
			t.Fatalf("regular formula not promoted: %s", got)
		}
		if got := git("--git-dir", origin, "ls-tree", "--name-only", "main", "Formula/codelima-beta.rb"); got != "" {
			t.Fatalf("retired beta formula still present: %s", got)
		}
