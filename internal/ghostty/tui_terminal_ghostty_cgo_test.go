...
func TestStartWiresPolloutWaiterForBackpressure(t *testing.T) {
	...
	ghostty.mu.Lock()
	waiter := ghostty.ptyWriter.WritableWaiter()
	ghostty.mu.Unlock()
	if waiter == nil {
...
func TestGhosttyTerminalHandoffReplaysAtCapturedGeometry(t *testing.T) {
	...
	lines := strings.Split(snapshotText(result.Snapshot), "\n")
...
func TestGhosttyKeyEncoderMatchesExistingCommonSequences(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "native-shift-enter",
			key: vaxis.Key{
				Keycode:   vaxis.KeyEnter,
				Modifiers: vaxis.ModShift,
			},
			// Current upstream distinguishes modified Enter by default. Keep
			// native semantics rather than a handwritten legacy-key fallback.
			want: "\x1b[27;2;13~",
		},
		{
			name: "paste-text",
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			terminal := newCaptureTestTerminal(t, "native-keys")
			defer terminal.Close()
			if tc.cursorKeysApplication {
				terminal.ingestPTY([]byte("\x1b[?1h"))
			}
			if tc.applicationKeypad {
				terminal.ingestPTY([]byte("\x1b="))
			}
			got := encodeTUITerminalKeyWithGhostty(tc.key, terminal.keyEncoder, terminal.term, tc.applicationKeypad, tc.cursorKeysApplication)
			if got != tc.want {
...
func TestGhosttyKeyEncoderTranslatesLegacyFunctionKeyAliases(t *testing.T) {
	t.Parallel()
	...
	if got != "\x1B[1;5Q" {
		t.Fatalf("native encoder did not preserve the control-F2 alias, got %q", got)
	}
}
...
func TestGhosttyMouseEncoderNeverUsesHandwrittenFallback(t *testing.T) {
	t.Parallel()
	...
	if got != "" {
		t.Fatalf("missing required mouse encoder must not fabricate VT, got %q", got)
	}
}
...
func TestGhosttyNativeLogBufferIsBoundedAndRecoversAfterDrain(t *testing.T) {
	ghosttyStderrCaptureMu.Lock()
	defer ghosttyStderrCaptureMu.Unlock()

	terminal := newCaptureTestTerminal(t, "bounded-native-log")
	defer terminal.Close()
	_ = readGhosttyTestLog(128 * 1024)
	defer func() { _ = readGhosttyTestLog(128 * 1024) }()

	done := make(chan struct{})
	go func() {
		defer close(done)
		emitGhosttyTestLog(bytes.Repeat([]byte("oversized"), 256*1024))
		message := bytes.Repeat([]byte("diagnostic"), 100)
		for range 10000 {
	...
	case <-done:
	case <-time.After(5 * time.Second):
		t.Fatal("native log callback blocked when no consumer drained its buffer")
	}
	buffered := readGhosttyTestLog(128 * 1024)
	if len(buffered) == 0 || len(buffered) > 64*1024 {
		t.Fatalf("native diagnostic buffer retained %d bytes; expected a nonempty bounded sink", len(buffered))
	}
	if bytes.Contains(buffered, []byte("oversized")) {
		t.Fatal("oversized diagnostic was retained instead of dropped as a whole record")
	}
	if residual := readGhosttyTestLog(128 * 1024); len(residual) != 0 {
		t.Fatalf("drain left %d buffered bytes", len(residual))
	}
	emitGhosttyTestLog([]byte("after-drain-marker"))
	if recovered := readGhosttyTestLog(128 * 1024); !bytes.Contains(recovered, []byte("after-drain-marker")) {
		t.Fatalf("native log sink did not recover after saturation: %q", recovered)
	}
}
