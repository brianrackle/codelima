...
type Vaxis struct {
	...
	// CodeLima: multipart Kitty transfers must not interleave on one surface.
	encodedUpload   *EncodedKittyImage
	reqCursorPos    bool
...
func (vx *Vaxis) render() {
	...
	// CodeLima: text may redraw during an incremental upload, but no other
	// Kitty command may interrupt its multipart transfer. Preserve the last
	// placement cut and reconcile graphics after the final chunk.
